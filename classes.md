...
(2.2) Zaimplementuj klasę `Student` dziedziczącą po klasie `Person` (z ćwiczenia 1.), która dodatkowo posiada pole `school` przechowujące nazwę szkoły. Klasa `Student` powinna posiadać metodę `Student.greet()`, która zwraca komunikat `My name is <name> and I am going to <school>`.
